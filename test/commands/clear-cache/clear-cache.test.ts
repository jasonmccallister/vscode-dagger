...
describe('Clear Cache Command', () => {
    ...
    beforeEach(() => {
        ...
        commandSpy = sandbox.stub(vscode.commands, 'registerCommand').callsFake((command: string, callback: any) => {
    ...
    it('should register the clear cache command', () => {
        assert.ok(commandSpy.calledWith('dagger.clearCache'), 'Command should be registered');
    });
    ...
    it('should not clear cache when not confirmed', async () => {
        ...
        assert.ok(mockCli.clearCache.notCalled, 'CLI clearCache should not be called');
    });
