...
describe('Dagger CLI Wrapper', () => {
    ...
    describe('fetchFunctionsList module relationship detection', () => {
        ...
        it('should handle single module without nesting', async () => {
            ...
            // Verify the results
            assert.strictEqual(functions.length, 2, 'Should return 2 function objects');
            ...
            functions.forEach((func: FunctionInfo) => {
                assert.strictEqual(func.isParentModule, false, 'Single module functions should not be marked as parent modules');
                assert.strictEqual(func.module, 'simple-module', 'Single module functions should have the module name');
                assert.strictEqual(func.parentModule, undefined, 'Single module functions should not have a parent');
            });
        });
        ...
        it('should ensure root module is never nested even with multiple modules', async () => {
            ...
            const rootModuleFunctions = functions.filter((f: FunctionInfo) => f.isParentModule);
            assert.ok(rootModuleFunctions.length > 0, 'Should have parent module functions');
            
            const rootBuildFunction = functions.find((f: FunctionInfo) => f.name === 'build');
            assert.ok(rootBuildFunction, 'Should find the root build function');
            assert.strictEqual(rootBuildFunction?.isParentModule, true, 'Root function should be parent module');
            assert.strictEqual(rootBuildFunction?.module, '', 'Root function should have empty module name');
            assert.strictEqual(rootBuildFunction?.parentModule, undefined, 'Root function should not have a parent');
            
            // Check submodules are properly nested under root
            const cliFunction = functions.find((f: FunctionInfo) => f.name === 'install');
            assert.ok(cliFunction, 'Should find the CLI install function');
            assert.strictEqual(cliFunction?.isParentModule, false, 'CLI function should not be parent module');
            assert.strictEqual(cliFunction?.module, 'cli', 'CLI function should have "cli" as module name');
            assert.strictEqual(cliFunction?.parentModule, 'app', 'CLI function should have "app" as parent');
            
            const testingFunction = functions.find((f: FunctionInfo) => f.name === 'run-tests');
            assert.ok(testingFunction, 'Should find the testing function');
            assert.strictEqual(testingFunction?.isParentModule, false, 'Testing function should not be parent module');
            assert.strictEqual(testingFunction?.module, 'testing', 'Testing function should have "testing" as module name');
            assert.strictEqual(testingFunction?.parentModule, 'app', 'Testing function should have "app" as parent');
            
            // Check independent module (Database) is treated as standalone (since it has no submodules)
            const databaseFunction = functions.find((f: FunctionInfo) => f.name === 'migrate');
            assert.ok(databaseFunction, 'Should find the database migrate function');
            assert.strictEqual(databaseFunction?.isParentModule, false, 'Independent module function should not be parent module when other hierarchy exists');
            assert.strictEqual(databaseFunction?.module, 'database', 'Independent module function should have its module name');
            assert.strictEqual(databaseFunction?.parentModule, undefined, 'Independent module function should not have a parent');
        });
        ...
        it('should handle complex nested module hierarchies correctly', async () => {
            ...
            const rootFunction = functions.find((f: FunctionInfo) => f.name === 'deploy');
            assert.ok(rootFunction, 'Should find the root deploy function');
            assert.strictEqual(rootFunction?.isParentModule, true, 'Root function should be parent module');
            assert.strictEqual(rootFunction?.module, '', 'Root function should have empty module name');
            assert.strictEqual(rootFunction?.parentModule, undefined, 'Root function should not have a parent');
            
            // First level submodules
            const backendFunction = functions.find((f: FunctionInfo) => f.name === 'build-api');
            assert.ok(backendFunction, 'Should find the backend function');
            assert.strictEqual(backendFunction?.isParentModule, true, 'Backend function should be parent module (has submodules)');
            assert.strictEqual(backendFunction?.module, '', 'Parent module function should have empty module name');
            assert.strictEqual(backendFunction?.parentModule, undefined, 'Backend function should not have a parent (is itself a parent)');
            
            const frontendFunction = functions.find((f: FunctionInfo) => f.name === 'build-app');
            assert.ok(frontendFunction, 'Should find the frontend function');
            assert.strictEqual(frontendFunction?.isParentModule, false, 'Frontend function should not be parent module (has no submodules)');
            assert.strictEqual(frontendFunction?.module, 'frontend', 'Frontend function should have "frontend" as module name');
            assert.strictEqual(frontendFunction?.parentModule, 'platform', 'Frontend function should have "platform" as parent');
            
            // Deeply nested module (PlatformBackendDb should be treated as submodule of PlatformBackend)
            const dbFunction = functions.find((f: FunctionInfo) => f.name === 'setup-database');
            assert.ok(dbFunction, 'Should find the database function');
            assert.strictEqual(dbFunction?.isParentModule, false, 'Database function should not be parent module');
            assert.strictEqual(dbFunction?.module, 'db', 'Database function should have "db" as module name (suffix after Backend)');
            assert.strictEqual(dbFunction?.parentModule, 'platform-backend', 'Database function should have "platform-backend" as parent');
        });
        ...
        it('should handle queryFunctionByID for root module functions', async () => {
            ...
            // Call the method under test
            const functionInfo = await (cli as any).fetchFunctionByID('func1', '/test/workspace');
            
            // Verify the results
            assert.ok(functionInfo, 'Should return function info');
            assert.strictEqual(functionInfo.name, 'build-image', 'Function name should be kebab-cased');
            assert.strictEqual(functionInfo.functionId, 'func1', 'Function ID should match');
            assert.strictEqual(functionInfo.isParentModule, true, 'Root module function should be marked as parent module');
            assert.strictEqual(functionInfo.module, '', 'Root module function should have empty module name');
            assert.strictEqual(functionInfo.parentModule, undefined, 'Root module function should not have a parent');
            assert.strictEqual(functionInfo.returnType, 'container', 'Return type should be converted to friendly name (lowercase)');
        });
        ...
        it('should handle queryFunctionByID for submodule functions', async () => {
            ...
            // Call the method under test
            const functionInfo = await (cli as any).fetchFunctionByID('func2', '/test/workspace');
            
            // Verify the results
            assert.ok(functionInfo, 'Should return function info');
            assert.strictEqual(functionInfo.name, 'dagger-dev-cli.install-binary', 'Function name should include module prefix and be kebab-cased');
            assert.strictEqual(functionInfo.functionId, 'func2', 'Function ID should match');
            assert.strictEqual(functionInfo.isParentModule, false, 'Submodule function should not be marked as parent module');
            assert.strictEqual(functionInfo.module, 'cli', 'Submodule function should have clean module name');
            assert.strictEqual(functionInfo.parentModule, undefined, 'Parent module info is not available in this context');
            assert.strictEqual(functionInfo.returnType, 'file', 'Return type should be converted to friendly name (lowercase)');
        });
        ...
        it('should ensure root module functions never have non-empty module names (tree view requirement)', async () => {
            ...
            const parentModuleFunctions = functions.filter((f: FunctionInfo) => f.isParentModule);
            ...
            parentModuleFunctions.forEach((func: FunctionInfo) => {
                assert.strictEqual(func.module, '', `Parent module function "${func.name}" must have empty module name for tree view root display, got: "${func.module}"`);
                assert.strictEqual(func.parentModule, undefined, `Parent module function "${func.name}" must not have a parent module`);
            });
            
            // Verify that root module (MainApp) is identified correctly
            const rootFunction = functions.find((f: FunctionInfo) => f.name === 'deploy');
            assert.ok(rootFunction, 'Should find the root deploy function');
            assert.strictEqual(rootFunction.isParentModule, true, 'Root function should be marked as parent module');
            assert.strictEqual(rootFunction.module, '', 'Root function must have empty module name for tree view');
            
            // Verify that submodules have proper module names (non-empty)
            const submoduleFunctions = functions.filter((f: FunctionInfo) => !f.isParentModule);
            submoduleFunctions.forEach((func: FunctionInfo) => {
                assert.notStrictEqual(func.module, '', `Submodule function "${func.name}" must have non-empty module name, got: "${func.module}"`);
            });
            
            // Verify specific submodule functions
            const backendFunction = functions.find((f: FunctionInfo) => f.name === 'build-backend');
            assert.ok(backendFunction, 'Should find the backend function');
            assert.strictEqual(backendFunction.isParentModule, false, 'Backend function should not be parent module');
            assert.strictEqual(backendFunction.module, 'backend', 'Backend function should have "backend" as module name');
            
            const frontendFunction = functions.find((f: FunctionInfo) => f.name === 'build-frontend');
            assert.ok(frontendFunction, 'Should find the frontend function');
            assert.strictEqual(frontendFunction.isParentModule, false, 'Frontend function should not be parent module');
            assert.strictEqual(frontendFunction.module, 'frontend', 'Frontend function should have "frontend" as module name');
            
            // Verify standalone service (should be treated as a regular module, not parent, since MainApp is the parent)
            const standaloneFunction = functions.find((f: FunctionInfo) => f.name === 'run-service');
            assert.ok(standaloneFunction, 'Should find the standalone service function');
            assert.strictEqual(standaloneFunction.isParentModule, false, 'Standalone service should not be parent module when other hierarchy exists');
            assert.strictEqual(standaloneFunction.module, 'standalone-service', 'Standalone service should have its module name');
        });
    });
});
