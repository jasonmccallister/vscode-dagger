...
describe('Tree Provider', () => {
    ...
    it('should display root module functions at top level with multiple modules', async () => {
        ...
        assert.strictEqual(rootFunctions.length, 2, 'Should have 2 root functions at top level');
        assert.ok(rootFunctions.some(fn => fn.label === 'rootFunc1'), 'Should find rootFunc1 at top level');
        assert.ok(rootFunctions.some(fn => fn.label === 'rootFunc2'), 'Should find rootFunc2 at top level');
        
        // Verify module items exist
        assert.strictEqual(moduleItems.length, 2, 'Should have 2 module items');
        assert.ok(moduleItems.some(mod => mod.label === 'submodule1'), 'Should find submodule1');
        assert.ok(moduleItems.some(mod => mod.label === 'other-module'), 'Should find other-module');
        
        // Verify module children
        const submodule = moduleItems.find(mod => mod.label === 'submodule1') as DaggerTreeItem;
        if (submodule && submodule.children) {
            assert.strictEqual(submodule.children.length, 2, 'Submodule should have 2 functions');
            const functionNames = submodule.children.map(item => item.label);
            assert.ok(functionNames.includes('subFunc1'), 'Should find subFunc1 in submodule');
            assert.ok(functionNames.includes('subFunc2'), 'Should find subFunc2 in submodule');
        } else {
